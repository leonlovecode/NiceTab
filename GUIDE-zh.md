...
| 发送标签页时-以下域名的标签页不会发送到管理后台 | 某些域名的标签页，如：空白页、新标签页等重要性低的标签页不想发送到管理后台，可填写对应的域名，按照提示信息配置 <br /> 正则示例：`.*?\.vuejs\.org.*` | 空白页和新标签页的地址 | ★★ |
...
| 配置项 | 说明 | 默认值 | 关键指数 |
| --- | --- | --- | --- |
| 网页标题自定义 | 自定义网页标题，该配置项可以改写网页的标题。支持多种匹配模式，（如**相同**，**开始于**，**结束于**，**包含**，**正则**）。<br />正则示例：`.*?\.vuejs\.org.*` | 无 | ★★★★★ |
